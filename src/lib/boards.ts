...
/** The current user's saved canvas positions for boards, keyed by boardId. */
export async function getBoardPositions(userId: string) {
  const rows = await prisma.boardOrder.findMany({
    where: { userId },
    select: { boardId: true, x: true, y: true },
  });
  return new Map(rows.map((r) => [r.boardId, { x: r.x, y: r.y }]));
}
